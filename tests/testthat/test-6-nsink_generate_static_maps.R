...
test_that("static maps have positive values", {
  expect_gte(min(terra::values(niantic_static_maps$removal_effic), na.rm = TRUE), 0)
  expect_gte(min(terra::values(niantic_static_maps$loading_idx), na.rm = TRUE), 0)
  expect_gte(min(terra::values(niantic_static_maps$transport_idx), na.rm = TRUE), 0)
  expect_gte(min(terra::values(niantic_static_maps$delivery_idx), na.rm = TRUE), 0)
})
